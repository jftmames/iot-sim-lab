...
def main():
    ...
    try:
        while True:
            payloads = [
                (f"{cfg['base_topic']}/temp", {"t": ts(), "type":"temperature", "unit":"C", "value": temp.read()}),
                (f"{cfg['base_topic']}/hum",  {"t": ts(), "type":"humidity",    "unit":"%", "value": hum.read()}),
                (f"{cfg['base_topic']}/prox", {"t": ts(), "type":"proximity",   "unit":"cm","value": prox.read()}),
            ]
